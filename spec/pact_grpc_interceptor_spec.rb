...
RSpec.describe PactGrpcRuby::PactGrpcInterceptor do
  ...
  it 'sends a Pact interaction and yields' do
    expect { |b| interceptor.request_response(request: request, call: call, method: method, metadata: metadata, &b) }.to yield_control
  end
  ...
  it 'handles errors when sending Pact interaction' do
    ...
    expect { interceptor.request_response(request: request, call: call, method: method, metadata: metadata) }.to raise_error(StandardError)
  end
end